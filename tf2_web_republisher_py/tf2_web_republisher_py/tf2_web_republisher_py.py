...
class ClientGoalInfo(ClientInfo):
    def __init__(
        self, handle, client_id: int, tf_subscriptions: List[TFPair] = [], timer=None
    ):
...
class TFRepublisher(Node):
    ...
    def __init__(self):
        ...
        self.republish_service = self.create_service(
            RepublishTFs, "republish_tfs", self.request_cb
        )
        self.active_clients = {}
    ...
    def goal_cb(self, goal_handle):
        ...
        if goal_rate == 0:
            ...
            goal_rate = 10.0
        goal_info.timer = self.create_timer(
            1.0 / goal_rate, lambda: self.process_goal(client_id)
        )
    ...
    def cancel_cb(self, goal_handle):
        ...
        if client_id_to_be_canceled:
            ...
            self.get_logger().info(
                "Cancelling action with id" + str(client_id_to_be_canceled)
            )
            return rclpy.action.CancelResponse.ACCEPT
        else:
            self.get_logger().info(
                "Cancel callback: not found action with id" + str(goal_handle)
            )
    ...
    def request_cb(self, request, response):
        ...
        if request_rate == 0:
            ...
            request_rate = 10.0
        request_info.timer = self.create_timer(
            1.0 / request_rate, lambda: self.process_request(client_id)
        )
        if not self.active_clients:
            self.tf_buffer = Buffer()
            self.tf_listener = TransformListener(self.tf_buffer, node=self)
        ...
        self.get_logger().info(
            "Publishing requested TFs on topic {0}".format(topic_name)
        )
        return response
    ...
    def set_subscriptions(
        ...
        for source_frame in source_frames:
            tf_pair = TFPair(
                self.clean_tf_frame(source_frame),
                self.clean_tf_frame(target_frame),
                angular_thres,
                trans_thres,
            )
    ...
    def process_request(self, client_id: str):
        ...
        try:
            ...
            self.get_logger().info(
                "Trying to access removed request: {}".format(client_id)
            )
            return
    ...
    def process_goal(self, client_id: str):
        ...
        if len(tf_array) > 0:
            # publish TFs
            self.get_logger().info(
                "Client {0} TFs feedback published:".format(request.client_id)
            )
            feedback.transforms = tf_array
            request.handle.publish_feedback(feedback)
        else:
            self.get_logger().info(
                "Client {0}: No TF frame update needed:".format(request.client_id)
            )

    def update_subscriptions(
        self, tf_subscriptions: List[TFPair]
        ...
        for pair in tf_subscriptions:
            try:
                ...
                transform = self.tf_buffer.lookup_transform(
                    pair.target_frame, pair.source_frame, rclpy.time.Time()
                )
                # If the transform broke earlier, but worked now (we didn't get
                # booted into the catch block), tell the user all is well again
                if not pair.is_ok:
                    self.get_logger().info(
                        "Transform from {0} to {1} is working again".format(
                            pair.source_frame, pair.target_frame
                        )
                    )
                    pair.is_ok = True
